$(document).ready(function () {
    $.getJSON("https://api.openweathermap.org/data/2.5/weather?id=1880252&units=metric&appid=03e4c77b5a50a7bcbd540fb044c1e2d1", function(data) {
...
function getTheNearestAirport(lat,lon){
    var settings = {
        "async": true,
        "crossDomain": true,
        "url": "https://aerodatabox.p.rapidapi.com/airports/search/location/"+lat+"/"+lon+"/km/100/1?withFlightInfoOnly=true",
        "method": "GET",
        "headers": {
...
$.ajax(settings).done(function (response) {
   ...
   var settings = {
	"async": true,
	"crossDomain": true,
	"url": "https://world-time2.p.rapidapi.com/timezone/Asia/Singapore",
	"method": "GET",
	"headers": {
...
$.ajax(settings).done(function (response) {
   ...
   var settings = {
    "url": "https://api.imgur.com/3/album/HJqJ9FO/images",
    "method": "GET",
    "timeout": 0,
    "headers": {
